...
header_str = "@M05774:236:000000000-LKC5Y:1:1107:27241:{index} {dir}:N:0:GATCAGAT+TCCGCGAA"
    ...
    with open('single_deletion_forward.fastq', "w") as f1:
        with open('single_deletion_reverse.fastq', "w") as f2:
            for index in range(len(sequence) - DELETION_COUNT):
                header_forward = header_str.format(index=index, dir=1)
                header_backward = header_str.format(index=index, dir=2)
