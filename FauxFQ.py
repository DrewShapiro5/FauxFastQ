...
def main():
    ...
    if args.reference_forward:
        ...
        qual_string_forward = quality_score_approximation.get_average_quality(str(reference_forward))
        print(f'Average quality for forward read:\n{qual_string_forward}\n')
    qual_string_reverse = ''
    if args.reference_reverse:
        reference_reverse = args.reference_reverse
        qual_string_reverse = quality_score_approximation.get_average_quality(str(reference_reverse))
    ...
    # If in append mode, delete the previous contents of the output files
    if args.append:
    ...
    if args.deletion:
        ...
        fqe.create_simulated_fastqs_deletion(sequence=reference_sequence,
                                             ...
                                             # TODO: make default output file names nice (based on the reference file name?)
                                             file_forward=args.output_forward if args.output_forward else None,
                                             file_reverse=args.output_reverse if args.output_reverse else None,)
        total_edit_modes += 1
    if args.insertion:
        # TODO: for runs without quality score, specify read length
